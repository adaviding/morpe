...
namespace Morpe
{
	...
	public class MonotonicRegressor
	{
		...
		public int Run(double[] output, double[] input)
		{
			...
			{
				dy = new double[input.Length];
				ynd = new double[input.Length];
			}
			//	Count the number of trips through the loop.
			int nTrips = 0;
			int tripLimit = 100 + (int)(30.0 * Math.Log((double)input.Length));
			int n = input.Length;
			...
			while (dyMin<tolY)
			{
				...
				while (j>0)
				{
					...
					if (halfThis<0.0)
					{
						...
						dy[i] = 0.0; // The difference between y(i) and y(i+1) goes to zero.
					}
					halfThis = dy[j]/2.0;
					if (halfThis<0.0)
					{
						...
						dy[j] = 0.0; // The difference between y(j) and y(j+1) goes to zero.
					}
					j--;
					i++;
				}
			...
			if (this.Type==MonotonicRegressionType.Blended)
			{
				...
				blendFactor = 1.0/(1.0+   Math.Pow( (double)(4*maxLen)/(double)(n-1), 3.0 )   );
			}
					...
					while( iMid<nm1 )
					{
						...
						if( iHigh-iMid>1 )
						{
							if( iMid-iLow>1 )
							{
								...
								dyThis = dy[iMid]/(double)(iHigh-iLow);
							...
							else
							{
								...
								dyThis = dy[iMid]/(double)(iHigh-iMid);
								dy[iMid] = dyThis;
