...
% Elements of the function 'parseStruct' and 'val2str' were edited by A. David Ing, 5-10-2011
...
function [] = parseStruct(s,docNode,curNode,pName)
    ...
    for i = 1:length(fnames)
        ...
        elseif (strcmp(curfield,'Text'))
            ...
            if (isstruct(s.(curfield)))
				%structure elements
				for j=1:numel(s.(curfield))
					curElement = docNode.createElement(curfield);
					curNode.appendChild(curElement);
					parseStruct(s.(curfield)(j),docNode,curElement,[pName curfield '.'])
				end
